...
describe('UserService', () => {
  ...
  it('should be return error in findUserById (Error request)', async () => {
  ...
  it('should be return error in findUserByEmail (Error request)', async () => {
    jest.spyOn(userRepository, 'findOne').mockRejectedValueOnce(new Error()); 
    expect(service.findUserByEmail(userEntityMock.email)).rejects.toThrowError();
  });
  ...
  it('should be return error in getUserByIdWuthRelations', async () => {
   const user = await service.getUserByIdWuthRelations(userEntityMock.id);
    expect(user).toEqual(userEntityMock);
  });

  it('should be return error if user exist', async () => {
    expect(service.createUser(createUserMock)).rejects.toThrowError();
  });

  it('should be return user if user not exist', async () => {
    jest.spyOn(userRepository, 'findOne').mockResolvedValue(undefined);
